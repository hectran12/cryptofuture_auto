...
def driver_create ():
    op = Options()
    conf = getConfig()["profile_setup"]
    # add extension
    op.add_argument('--user-data-dir={0}'.format(conf["path_profile"]))
    op.add_argument('--profile-directory=Profile {0}'.format(conf["profile_number"]))
...
def login (driver, wallet):
    if driver.page_source.find('Checking if the site connection is secure') > -1:
        
        print(color.YELLOW, 'cloudflare....')
        driver.delete_all_cookies()
        print('Please enter if you pass cloudflare')
        input()
        return False
    if driver.page_source.find('Login') == -1:
        ...
        driver.execute_script('''document.querySelector("#address").value = \'\'''')
        time.sleep(2)
...
def solveRecaptchav2GetTask ():
    data = json.dumps({
        'websiteURL': 'https://cryptofuture.co.in/',
        'websiteKey': '6Le53HQgAAAAAFBRUdmXaUSC_dRmxSSeP9nUlEZ9',
        'isInvisible': 'false'
    }).encode()
    bs64 = str(base64.b64encode(data)).replace("b'", '').replace("'", '')
    url = 'https://tronghoa.dev/free-bypass-captcha/solve.php?type=recaptchav2&data={0}'.format(bs64)
...
tags = {
    'btc' : '/bitcoin/?r=1Dotw12s2ed34wpXyh5gFwE29edbgemPpd',
    'doge' : '/dogecoin/?r=DJKntdmVqxZwJjBhRsZtsfaHmAAFbHAaEd',
    'ltc': '/litecoin/?r=MR4RsuJnNXWgfp9V6898VxZCzBLrVrz2MT',
    'trx': '/tron/?r=TPw8AibYuVVXQUM6UcsvmyUaL6S4wAj6kL',
    'sol' : '/solana/?r=D1MPLyFWEr4o4jzksF7gAu56KAx5gdtMnkEQPBNReY1o',
    'dash' : '/dashcoin/?r=XrDoxcRinJkcrHhASdUkTJ8WuRrqMtBsyY'
}
    ...
    for x in tags:
        for y in wallet:
            if x == y:
                while True:
                    try:
                        drivers.get('https://cryptofuture.co.in' + tags[x])
